...
var ASM_CONSTS = {
  685276: ($0, $1) => { const audioContext = emscriptenGetAudioObject($0); const audioWorkletNode = emscriptenGetAudioObject($1); audioWorkletNode.connect(audioContext.destination); const startButton = document.createElement('button'); startButton.innerHTML = 'Toggle playback'; document.body.appendChild(startButton); startButton.onclick = () => { if (audioContext.state !== 'running') audioContext.resume(); else audioContext.suspend(); }; if (navigator.requestMIDIAccess) navigator.requestMIDIAccess().then(midi => { midi.inputs.forEach(entry => entry.onmidimessage = event => { if (event.data.length === 3) { Module._playMsg((event.data[2] << 16) + (event.data[1] << 8) + event.data[0]); } else console.log(event.data); }); }).catch(console.error); },  
 686009: ($0) => { console.log(Module.UTF8ToString($0).replace(//g, '*')); }
};
...
// end include: preamble.js
  ...
  var _emscripten_start_wasm_audio_worklet_thread_async = (contextHandle, stackLowestAddress, stackSize, callback, userData) => {
      ...
      audioWorklet.addModule('webmt32.aw.js').then(() => {
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 668404;
